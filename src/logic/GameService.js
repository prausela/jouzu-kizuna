...
const getNextQuestion = async (categoryId, setId, number_of_answers, gameData) => {
    ...
    const wrongAnswers = getXDifferentFrom(Math.min(question.answers.length, number_of_answers), question.answers);
    const correctAnswerPosition = RandomService.getRandomInt(0, Math.min(question.answers.length+1, number_of_answers));
    return {
        id       : question.id,
